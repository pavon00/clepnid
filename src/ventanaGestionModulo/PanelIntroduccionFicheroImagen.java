...
public class PanelIntroduccionFicheroImagen extends Composite {
	...
	public PanelIntroduccionFicheroImagen(PanelConfiguracionModuloFichero panel, int style, String labelText,
	...
	public PanelIntroduccionFicheroImagen(PanelConfiguracionModuloIcono panel, int style, String labelText,
			int width, Ventana ventana) {
		super(panel, style);
		this.ventana = ventana;
	...
	public PanelIntroduccionFicheroImagen(PanelConfiguracionModuloVista panel, int style, String labelText,
			int width, Ventana ventana) {
		super(panel, style);
		this.ventana = ventana;
		...
		btn.addListener(SWT.Selection, new Listener() {
			public void handleEvent(Event event) {
				if (comprobarCarpetaProyecto()) {
					...
					String[] extensiones = new String[ArraysStringFormatos.IMAGEN.length + 1];
					int contador = 1;
					for (int i = 0; i < ArraysStringFormatos.IMAGEN.length; i++) {
						extensiones[contador] = "*." + ArraysStringFormatos.IMAGEN[i];
						contador++;
					}
					extensiones[0] = "*.*";
					...
					if (file != null) {
						...
						if (comprobarIndexHtml(fichero)) {
							...
							MessageDialog.openError(ventana.getShell(), "Error",
									"Debe seleccionar un fichero imagen dentro de la carpeta proyecto");
						}
					}
				} else {
					MessageDialog.openError(ventana.getShell(), "Error",
	...
	public void rellenarTextoIndexHtml(PanelConfiguracionModuloFichero panel, File fichero) {
		String rutaProyecto = new File(panel.getSeleccionarProyecto().getTextoIntroducido()).getAbsolutePath();
		String rutaIndice = fichero.getAbsolutePath();
		String rutaAux = getStringSobrante(rutaProyecto,rutaIndice).replace("\\", "/");
		System.out.println(rutaProyecto+", "+rutaIndice+", "+rutaAux);
		texto.setText(rutaAux);
