...
public class PanelConfiguracionModuloFichero extends Composite {

	private Ventana ventana;
	private Boolean variosArchivos;
	...
	private PanelIntroduccionTextos titulo, textoBoton, rutaHttp, reemplazarTexto;
	private PanelSeleccionExtensiones panelFormato;
	private Button btnCrear, btnGrupo;

	public PanelConfiguracionModuloFichero(Composite parent, int style, Ventana ventana) {
		super(parent, style);
		this.setVentana(ventana);
		this.variosArchivos = false;
		this.setBackground(this.getDisplay().getSystemColor(SWT.COLOR_WHITE));
		this.setLayout(new GridLayout(1, false));
		GridData data = new GridData(SWT.FILL, SWT.FILL, true, true);
		data.widthHint = 400;
		data.verticalIndent = 10;
		btnGrupo = new Button(this, SWT.CHECK);
		btnGrupo.setText("Varios Archivos");
		btnGrupo.addListener(SWT.Selection, new Listener() {
		...
		btnGrupo.setBackground(this.getDisplay().getSystemColor(SWT.COLOR_WHITE));
		...
		reemplazarTexto = new PanelIntroduccionTextos(this, 0, "Reemplazar texto:", 200);
		reemplazarTexto.setLayoutData(dataTexto1);
		...
		panelFormato = new PanelSeleccionExtensiones(this, 0, ventana);
		panelFormato.setLayoutData(data);
		...
		btnCrear.addListener(SWT.Selection, new Listener() {
			public void handleEvent(Event event) {
				if (ComprobarInputs()) {
					...
					String escritura = "[\n\t{\n\t\t" + "\"Web\": {\n" + "\t\t\t\"Title\": \""
							+ titulo.getTextoIntroducido() + "\",\n" + "\t\t\t\"Extensions\": "
							+ Arrays.toString(panelFormato.getFormatosSeleccionados()) + ",\n"
							+ "\t\t\t\"BotonText\": \"" + textoBoton.getTextoIntroducido() + "\",\n"
							+ "\t\t\t\"HtmlBodyReplace\": \"" + reemplazarTexto.getTextoIntroducido() + "\",\n"
							+ "\t\t\t\"Group\": \"" + grupo + "\",\n" + "\t\t\t\"Html\": \""
							+ seleccionarIndex.getTextoIntroducido() + "\",\n" + "\t\t\t\"rutaHttp\": \"" + rutaHttpAux
							+ "\",\n" + "\t\t\t\"rutaImagen\": \"" + rutaImagen + "\"\n" + "\t\t}\n" + "\t}\n" + "]"
							+ "";
					...
					if (seleccionarIndex.getRutaPadre() != null) {
						...
						String stringRutaHtml = ventana.getPanelFicheroModulo().getRutaPadreIndex() + File.separatorChar
								+ "clepnid.json";
						File rutaHtml = new File(stringRutaHtml);
						if (rutaHtml.exists()) {
							if (MessageDialog.openQuestion(ventana.getShell(), "¿Sobreescribir?",
	...
	public boolean ComprobarInputs() {
		...
		if (reemplazarTexto.getTextoIntroducido().equals("")) {
			MessageDialog.openError(ventana.getShell(), "Error", "No se ha introducido el reemplazo de texto");
			return false;
		}
		...
		if (Arrays.toString(panelFormato.getFormatosSeleccionados()).length() == 0) {
			MessageDialog.openError(ventana.getShell(), "Error", "No se ha introducido extensiones");
			return false;
		}
	...
	public Boolean getVariosArchivos() {
		return variosArchivos;
	}

	public void setVariosArchivos(Boolean variosArchivos) {
		this.variosArchivos = variosArchivos;
	}
	...
	public PanelIntroduccionTextos getReemplazarTexto() {
		return reemplazarTexto;
	}

	public void setReemplazarTexto(PanelIntroduccionTextos reemplazarTexto) {
		this.reemplazarTexto = reemplazarTexto;
	}

	public PanelSeleccionExtensiones getPanelFormato() {
		return panelFormato;
	}

	public Button getBtnGrupo() {
		return btnGrupo;
	}

	public void setPanelFormato(PanelSeleccionExtensiones panelFormato) {
