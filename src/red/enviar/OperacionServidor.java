...
public class OperacionServidor extends Thread {
	...
	public byte[] doubleToByteArray(double numero) {
		byte[] b = null;
	...
	public OperacionServidor(Socket sc, String ip, ArrayList<File> ficheros) {
		this.sc = sc;
		conectado = true;
		this.ip = ip;
	...
	public void run() {
		...
		try {
			...
			if (din.readUTF().equals("si")) {
				...
				if (ficheros != null) {
					Ficheros files = new Ficheros(ficheros);
					dout.write(CreateDataPacket("123".getBytes("UTF8"),
							Serializar_funciones.longToBytes(files.tamanyoTodos)));
					dout.flush();
					dout.write(CreateDataPacket("121".getBytes("UTF8"),
							Configuracion.deserializar().serial.getBytes("UTF8")));
					dout.flush();
					for (File readFile : ficheros) {
						if (readFile.exists()) {
							dout.write(CreateDataPacket("122".getBytes("UTF8"),
									readFile.getAbsolutePath().getBytes("UTF8")));
							dout.flush();
							sendFileOutput(readFile);
						}
						if (readFile.isDirectory()) {
							String relativePath = readFile.getAbsoluteFile().getParentFile().getAbsolutePath()
									.replace("\\", "/");
